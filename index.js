   ...
   return [
    ...
    fetchURL3(args,util)//移動
    {
        if (mode == 1){
            ...
            if(raspi==1) fetch2(text);
            content=content+inst+text2+text3+'\n';
            //ステアリング補正時間
   ...
   fetchURL5(args,util)//粉
    {
        if ( mode == 1 )
            {   const server = "http://localhost:8000?";
             const inst= "inst=POWDER";//大文字
             const text = "&INOUT="+menu_4[Cast.toString(args.TEXT)];
             powdergo = server+inst+"&INOUT="+1;
             
             powderstop= server+inst+"&INOUT="+0; 
    ...
    fetchURL6(args,util)//回転
    {
        if ( mode == 1 ){
            ...
            if(raspi==1)  fetch2(text);
            content=content+inst+text1+text2+'\n';
...
fetchURL11 (args,util)
{
    ...
    { 
        ...
        if(flag==1 && flag2==1) 
        {
            ...
            if(raspi==1)  fetch2(text);
            content=content+inst+text1+'\n';
        
            inst= "inst=POWDER";//大文字
            text1 = "&INOUT="+1;
