...
//ar menu_2 = { [rot1]: 1, [rot2]: 2, [rot3]: 3 };//SPEED2
...
class Scratch3NewBlocks {
        ...
        return {
            ...
            blocks: [
                ...
                },
                {
                    ...
                    opcode: 'fetchURL19',
                    blockType: BlockType.COMMAND,
                    text: '円軌道：[GRID]に半径[RADIUS]m（範囲：' + min_radius / 100 + '~' + max_radius / 100 + '）の円軌道をさせる',
                    arguments: {
                        GRID: {
                            type: ArgumentType.STRING,
                            defaultValue: "時計回り",
                ...
                */
                ...
                },
                {
                    opcode: 'fetchURL22',
                    blockType: BlockType.REPORTER,
                    text: '【教員用】走行速度:[SPEED]cm/s 旋回速度:[SPEED2]度/s', 
                    arguments: {
                        SPEED: {
                            type: ArgumentType.NUMBER,
                            defaultValue: 13.6
    ...
    fetchURL3(args, util)//移動
    {
        if (write_enable == 1) {
            if (mode == 1) {
                ...
                point = 0;
                dx = this.move_speed * Math.cos(radians) / screen_mode;
                dy = this.move_speed * Math.sin(radians) / screen_mode;;
    ...
    **/
    ...
    fetchURL22 (args) {
        this.move_speed = Number(args.SPEED);
        this.turn_speed = Number(args.SPEED2);
        if (this.runtime) {
            this.runtime.requestRedraw();
        }
    }
